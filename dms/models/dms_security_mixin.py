...
class DmsSecurityMixin(models.AbstractModel):
    ...
    def _get_domain_by_inheritance(self, operation):
        ...
        domains = []
        # Get all used related records (19.0: read_group → _read_group)
    ...
    def _search(self, domain, *args, **kwargs):
        ...
        Odoo 19.0 changed ``ir.rule._compute_domain()`` to use
        ``Domain.optimize`` (basic) instead of ``Domain.optimize_full``, which
        means ``search=`` methods on non-stored computed fields like
        ``permission_read`` are silently bypassed when ``domain_force`` is
        evaluated. The read-side ``ir.rule`` records on ``dms.directory`` /
        ``dms.file`` would therefore no-op and every user would see every
        record. We restore the intended behaviour by AND-ing the same domain
        directly into the search here, where SQL translation runs the full
        optimizer that does honour ``search=`` methods.
