...
class DmsDirectory(models.Model):
    ...
    def _search_starred(self, operator, operand):
        # Odoo 19.0's basic Domain optimizer normalises ('starred', '=', True)
        # to ('starred', 'in', {True}), so handle both operators. Empty
        # collections are equivalent to a False match.
        if isinstance(operand, bool):
            truthy = operand
        else:
            try:
                truthy = True in operand
            except TypeError:
                truthy = bool(operand)
    ...
    def _inverse_starred(self):
        ...
        not_starred_records.write({"user_star_ids": [Command.link(self.env.uid)]})
        starred_records.write({"user_star_ids": [Command.unlink(self.env.uid)]})
