...
function _readStoredPreview() {
    ...
    // clicking "Hide preview" keeps it hidden on subsequent loads.
    try {
        return window.localStorage.getItem(DMS_LIST_PREVIEW_KEY) !== "0";
    } catch {
        return true;
    }
}
...
export class FileListRenderer extends ListRenderer {
    ...
    closePreview() {
        ...
        try {
            window.localStorage.setItem(DMS_LIST_PREVIEW_KEY, "0");
        } catch {
            // Persistence is best-effort.
        }
    }
    ...
    onCellClicked(record, column, ev) {
        ...
        if (record.resId) {
            ...
            try {
                window.localStorage.setItem(DMS_LIST_PREVIEW_KEY, "1");
            } catch {
                // Persistence is best-effort.
            }
            ev?.stopPropagation?.();
