...
function _instantiate() {
    ...
    // The renderer reads/writes density through `this._density`, a
    // useStoredState handle ({value} accessor backed by localStorage). The
    // hook is setup()-only, so inject a stand-in honouring the same contract:
    // `.value` round-trips through localStorage under DENSITY_KEY, which lets
    // us assert both the getter and the persistence side effect.
    inst._density = {
        get value() {
            return (
                window.localStorage.getItem(DENSITY_KEY) || DMS_KANBAN_DEFAULT_DENSITY
            );
        },
        set value(v) {
